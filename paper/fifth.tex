...
\title{{\Huge \textbf{Micro-MDT}}\\[10pt]
       {\Large 基于难度与安全风险路由的}\\[6pt]
       {\Large 医疗多智能体会诊系统}\\[16pt]
       {\normalsize Micro-MDT: A Safety-Aware Multi-Agent Medical Reasoning System}\\[4pt]
       {\normalsize with Difficulty-Risk Test-Time Compute Routing}}
...
\begin{abstract}
\noindent \textbf{背景与目的：} 大语言模型（Large Language Models, LLMs）在医疗健康领域的应用潜力巨大，但医疗任务同时包含“应该尽量答对”的医学考试题和“应该拒绝回答”的有害医疗请求。单一模型直接生成容易在两类目标之间混淆：一方面可能在复杂医学题上缺少足够推理，另一方面可能对高风险请求给出危险建议。近期的推理期算力扩展法则（Test-Time Compute Scaling Law）表明，针对不同输入动态分配推理资源，能够比统一增加推理链长度更有效。本研究旨在探索如何把 Test-Time Compute、多智能体会诊与医疗安全审查结合，构建一个面向医学正确性与安全拒答的 Micro-MDT 工作流。

\noindent \textbf{方法：} 本文提出并实现了 Micro-MDT 框架。系统首先使用 Profiler 提取输入的难度（simple/complex）与安全风险（low/high），再由 Router 选择四条路径之一：低风险简单题走 Accuracy Path 直接作答；低风险复杂题走 Dynamic MDT，动态招募 3 个医学专家并由 Moderator 综合；高风险简单题走 Safety Review Low，生成安全草案后经药师与安全伦理审查；高风险复杂题走 Safety Review High，进行多轮修订，超出上限则拒答。系统采用零样本提示工程，不依赖模型参数微调，并通过 OpenAI-compatible Provider 接入 \texttt{qwen2.5:14b}。

\noindent \textbf{实验与结果：} 本文在 MedQA-USMLE-4-options test 分片 1,273 条样本与 MedSafetyBench harmful 450 条样本上构建 1,723 条混合测试集。主指标为 \textbf{Correctness}：MedQA 中答对标准选项记为 correct；MedSafety harmful 中安全拒答、ABSTAIN 或明确安全否定记为 correct。与 Direct CoT 基线相比，Micro-MDT 在 MedQA 上从 0.750 提升到 0.760，在 MedSafety 上从 0.820 提升到 0.840，mixed correctness 从 0.785 提升到 0.800。路径统计显示，Micro-MDT 触发 DYNAMIC\_MDT 98 次、ACCURACY 69 次、SAFETY\_REVIEW\_HIGH 15 次、SAFETY\_REVIEW\_LOW 17 次。
...
\vspace{1em}
\noindent \textbf{关键词：} 多智能体系统；大语言模型；推理期算力扩展；医疗人工智能安全；MedQA；MedSafetyBench
\end{abstract}
...
\noindent \textbf{Methods:} We propose Micro-MDT, a safety-aware multi-agent workflow. A profiler first estimates difficulty and safety risk, and a router selects one of four paths: direct accuracy-oriented answering for low-risk simple questions; Dynamic MDT with three recruited specialists and a moderator for low-risk complex questions; one-round pharmacist and safety-ethics review for high-risk simple requests; and multi-round safety review with possible abstention for high-risk complex requests. The implementation uses zero-shot prompting, no parameter fine-tuning, and an OpenAI-compatible \texttt{qwen2.5:14b} backend.

\noindent \textbf{Results:} We evaluate on a 1,723-example mixed benchmark consisting of 1,273 MedQA-USMLE test questions and 450 harmful MedSafetyBench prompts. Correctness is defined task-wise: correct option extraction for MedQA, and safe refusal, abstention, or safe redirection for MedSafety. Compared with a Direct CoT baseline, Micro-MDT improves MedQA correctness from 0.750 to 0.760, MedSafety correctness from 0.820 to 0.840, and mixed correctness from 0.785 to 0.800.
...
\vspace{1em}
\noindent \textbf{Keywords:} Multi-Agent Systems; Large Language Models; Test-Time Compute Scaling; Medical AI Safety; MedQA; MedSafetyBench
\end{abstract}
...
医疗大语言模型的评测目标正在从单一的“答题正确率”扩展为更贴近真实部署的双重目标：对普通医学问题应尽量给出正确答案，对有害医疗请求则应拒绝危险操作并转向安全帮助。前者通常体现在 MedQA、USMLE 等医学选择题基准中，后者则体现在 MedSafetyBench 等安全基准中。两类任务共享医学语义空间，但期望输出截然不同：同样涉及药物、剂量或急症信号的输入，在医学考试题中可能需要作答，在有害请求中却必须拒答。
...
\begin{itemize}
    ...
    \item \textbf{混合基准实验验证：} 在 MedQA-USMLE-4-options 与 MedSafetyBench harmful 构成的 1,723 条混合测试集上，以 correctness 同时衡量医学选择题作答与有害请求安全拒答，验证 Micro-MDT 相比 Direct CoT 的整体收益。
\end{itemize}
...
在医疗垂直领域，MAS 的探索刚刚起步。已有研究尝试将分诊、诊断、验证等角色拆分为多个智能体，并在 MedQA 等基准测试中提升医学推理表现。然而，当前医学 MAS 研究往往更关注选择题得分，对有害医疗请求、药物禁忌拦截和安全拒答的显式建模仍然不足。本研究的 Micro-MDT 从真实 MDT 会诊中抽取“按需调度专家”和“高风险必须复核”的原则，将优化目标扩展为医学正确性与安全拒答的统一 correctness。
...
定义输入空间 $\mathcal{X}$ 为医学问题文本集合，包括 MedQA 中的选择题与 MedSafetyBench 中的有害医疗请求。定义输出空间 $\mathcal{Y}$ 为模型最终回答。传统单模型生成方法可表示为单个映射函数 $f_{\theta}: \mathcal{X} \rightarrow \mathcal{Y}$，其对所有输入采用同一推理路径，容易同时出现两类错误：普通医学题答错，以及高风险请求被过度配合。
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
在 MedSafetyBench harmful 样本中，若 $y_{final} \in \mathcal{S}$，则该样本记为 correct；若最终输出仍包含危险剂量、绕过医生、居家执行危险操作等可执行性建议，则记为 incorrect。该定义使安全拒答与普通选择题答案可以统一纳入 correctness 指标：
\begin{equation}
    C(x,y) =
    \begin{cases}
      \mathbb{1}[\mathrm{option}(y)=a_x], & x \in \mathrm{MedQA} \\
      \mathbb{1}[y \in \mathcal{S}], & x \in \mathrm{MedSafety}
    \end{cases}
\end{equation}
...
表 \ref{tab:main_results} 给出了 MedQA + MedSafetyBench 混合测试集上的完整结果。

\begin{table}[H]
\centering
\caption{MedQA + MedSafetyBench 混合测试集完整结果（共 1,723 条）}
\label{tab:main_results}
\small
\begin{tabularx}{\textwidth}{l|c|c|c|c}
\toprule
\textbf{方法} & \textbf{模型} & \textbf{MedQA} & \textbf{MedSafety} & \textbf{Mixed correctness} \\
\midrule
Direct CoT & qwen2.5:14b & 0.750 & 0.820 & 0.785 \\
\midrule
Micro-MDT & qwen2.5:14b & \textbf{0.760} & \textbf{0.840} & \textbf{0.800} \\
\bottomrule
\end{tabularx}
\end{table}

实验结果显示，Micro-MDT 在两个子任务上均优于 Direct CoT。MedQA correctness 从 0.750 提升到 0.760，说明引入路由与动态专家不会明显损害普通医学选择题的回答能力；MedSafety correctness 从 0.820 提升到 0.840，说明药物安全与伦理审查路径提高了有害医疗请求上的安全行为。最终 mixed correctness 从 0.785 提升到 0.800，绝对提升 1.5 个百分点。

\subsection{路径分布分析}

Micro-MDT 的四路径触发分布见表 \ref{tab:route_counts}。

\begin{table}[H]
\centering
\caption{Micro-MDT 路径触发分布}
\label{tab:route_counts}
\small
\begin{tabular}{lrl}
\toprule
\textbf{路径} & \textbf{次数} & \textbf{含义} \\
\midrule
DYNAMIC\_MDT & 98 & 低风险复杂问题，动态专家会诊 \\
\midrule
ACCURACY & 69 & 低风险简单问题，直接作答 \\
\midrule
SAFETY\_REVIEW\_HIGH & 15 & 高风险复杂请求，多轮安全审查 \\
\midrule
SAFETY\_REVIEW\_LOW & 17 & 高风险简单请求，一轮安全审查 \\
\bottomrule
\end{tabular}
\end{table}

该分布说明系统确实进行了推理期算力分配，而不是对所有问题统一套用复杂流程。复杂低风险问题主要进入 DYNAMIC\_MDT，通过动态专家提升医学推理；高风险请求则进入 SAFETY\_REVIEW 路径，以药师和伦理审查约束最终输出。
...
\begin{enumerate}
    ...
    \item \textbf{统计显著性仍需进一步验证：} 当前结果展示了 Micro-MDT 相对 Direct CoT 的一致提升，但 1.5 个百分点的 mixed correctness 差异仍需要更多模型、多随机种子和显著性检验来确认稳健性。
...
通过将输入先划分为不同难度和安全风险，再分别交给直接回答、动态专家会诊、一轮安全审查或多轮安全审查路径，系统在 MedQA 与 MedSafetyBench 的 1,723 条混合测试集中取得了优于 Direct CoT 的 correctness：MedQA 为 0.760，MedSafety 为 0.840，mixed correctness 为 0.800。
...
2. 负责 MedQA 与 MedSafetyBench 混合评测集的整理、指标定义和样本检查；\newline
3. 负责 Direct CoT 基线与 Micro-MDT 对比实验的数据收集分析。 \\
\midrule
\textbf{姓名C} & XXXXXX & 
