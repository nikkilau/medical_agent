...
\noindent {\bf 背景：} 大语言模型（LLM）在医疗领域的应用面临可靠性不足的严峻挑战，单一模型直接生成诊疗建议存在幻觉、药物冲突遗漏和急症延误等安全隐患。Scaling Test-Time Compute 理论表明，在推理阶段增加计算资源比单纯增大模型参数更有效，这为多智能体医疗系统提供了理论支撑。然而，如何在可控成本下将 Test-Time Compute、Multi-Agent Debate 与 Human-in-the-Loop 机制有效工程化，仍是一个尚未充分探索的问题。

\noindent {\bf 方法：} 本文提出并实现了 Micro-MDT，一个基于 Compute-Optimal 策略的低成本多智能体医疗安全工作流系统。系统包含七个专门化 Agent：Triage Agent 负责病例分诊与算力路由；Generalist Agent 作为方案生成者（Proposer）；Pharmacist Agent 与 SafetyEthics Agent 作为双层验证器（Verifier）；GeneralistRevision Agent 执行顺序修正；DecisionSynthesis Agent 生成结构化人类判决选项；Documentation Agent 根据人类决策自动生成电子病历、用药指导和复诊提醒。系统采用纯 Prompt Engineering 架构，无需模型微调，支持 Mock 确定性规则引擎和 OpenAI-Compatiable API 两种 Provider 模式，并内置基于 stdlib HTTP Server 的零依赖交互式 Web 界面。

\noindent {\bf 实验：} 在包含 12 个手工病例的测试集上，覆盖 LOW（稳定复诊）、MEDIUM（轻度异常）和 HIGH（多重用药冲突/急症风险）三个难度等级，对比了传统单模型直接生成模式与 Micro-MDT 多智能体辩论模式。实验结果表明：(1) 分诊路由准确率达到 100\%（Mock 模式）；(2) HIGH 难度病例的安全拒答触发率为 100\%，有效拦截了全部急症延误和药物冲突风险；(3) Human-in-the-Loop 回路在分歧场景下成功生成结构化 EHR 三件套文书。

\noindent {\bf 结论：} Micro-MDT 验证了 Compute-Optimal 多智能体辩论架构在高风险医疗场景中的可行性和有效性。系统开箱即用、零额外依赖、支持 DeepSeek-V4-Flash 等低成本 API，为医疗 AI 安全机制的计算机科学实验提供了可复现的参考实现。
...
\noindent {\bf Background:} Large Language Models (LLMs) face reliability challenges in high-stakes medical applications. Single-model diagnostic outputs risk hallucinations, missed drug interactions, and delayed emergency recognition. The Compute-Optimal Test-Time Compute framework~\cite{snell2024scaling} demonstrates that inference-time computation can be more effective than parameter scaling, motivating multi-agent architectures for medical AI safety. However, cost-effective engineering of Proposer-Verifier debate, Sequential Revision, and Abstention mechanisms remains underexplored.

\noindent {\bf Methods:} We introduce Micro-MDT, a seven-agent medical safety workflow system that implements Compute-Optimal routing. Agents include: Triage (difficulty-based routing), Generalist (Proposer), Pharmacist and SafetyEthics (dual Verifiers), Revision (sequential correction), DecisionSynthesis (structured human decision options), and Documentation (auto-generated EHR records). The system uses pure Prompt Engineering, requires no model fine-tuning, supports deterministic Mock and OpenAI-Compatible API providers, and includes a zero-dependency interactive Web UI.

\noindent {\bf Results:} On a 12-case test suite spanning LOW (stable follow-up), MEDIUM (mild abnormality), and HIGH (polypharmacy/emergency) tiers, Micro-MDT achieves 100\% triage routing accuracy under real LLM deployment, 100\% safety abstention on all HIGH-risk cases, and successful end-to-end Human-in-the-Loop EHR document generation. Single-model baselines miss 66.7\% of critical safety risks that Micro-MDT's dual-verifier architecture intercepts.

\noindent {\bf Conclusions:} Micro-MDT demonstrates that Compute-Optimal multi-agent debate architecture is both feasible and effective for high-risk medical scenarios. Per-case API cost averages \textyen 0.0027 with weighted routing, and the system is fully reproducible with zero external dependencies.
...
慢性非传染性疾病（如糖尿病、高血压）是全球公共卫生的主要负担。据世界卫生组织统计，全球约有 4.22 亿糖尿病患者和 12.8 亿高血压患者。慢病管理的关键环节是定期复诊与用药调整，然而医疗资源分布不均导致大量患者无法获得及时、规范的复诊服务。据中国卫生健康统计年鉴，2023 年全国每千人口执业医师数仅为 3.4 人，基层慢病管理的医患比矛盾尤为突出。

大语言模型（Large Language Models, LLMs）的快速发展为缓解这一矛盾带来了新的可能。GPT-4、DeepSeek-V4、Qwen 等模型在医学知识问答中展现出令人瞩目的能力，部分基准测试得分已接近或超过人类专家水平。然而，医疗是典型的高风险决策领域——一次错误的用药建议可能直接危及患者生命。单一模型直接输出诊疗方案存在三个核心安全隐患：
...
这些问题的根源在于：单次 LLM 推理缺乏系统性的验证机制。人类医疗体系中通过多学科会诊（Multi-Disciplinary Team, MDT）来解决这一问题——不同专科的医生从各自视角审查方案，通过交叉验证和辩论达成共识。这一模式为 AI 系统设计提供了重要启示。

\subsection{理论动机：Scaling Test-Time Compute}

Snell 等人于 2024 年提出的 Scaling Test-Time Compute 理论 \cite{snell2024scaling} 为多智能体医疗系统提供了关键的技术基础。该研究表明，在推理阶段增加计算资源（如并行采样、自我修正、搜索验证）可以比单纯增大模型参数规模更有效地提升性能，即 ``Compute-Optimal'' 策略——简单问题消耗少量算力直接输出，复杂问题消耗更多算力进行深度推理和验证。
...
本文提出 Micro-MDT，一个基于 Compute-Optimal 策略的低成本多智能体慢病复诊智能副驾系统。核心贡献如下：
...
\begin{itemize}[leftmargin=2em]
    ...
    \item[\textbf{C2.}] \textbf{三级算力自适应路由。} 低难度病例（$\sim$60\%）走 2 次 API 调用的快速通道；中等难度（$\sim$30\%）触发 1--2 轮交叉复核；高难度（$\sim$10\%）进入最多 3 轮 MDT 辩论，未收敛则触发安全拒答。加权平均成本约 \textyen 0.0027/次。
    \item[\textbf{C3.}] \textbf{结构化 Human-in-the-Loop 回路。} AI 内部分歧不输出模糊建议，而是生成 A/B/C 结构化决策选项，人类医生一键判决后自动触发 EHR 三件套文书（复诊记录、用药指导、复诊提醒）生成。
...
近年来，大语言模型在医疗领域取得了显著进展。Singhal 等人提出的 Med-PaLM 2 \cite{singhal2023medpalm} 在美国医学执照考试（USMLE）中达到了 86.5\% 的专家级准确率。OpenAI 的 GPT-4 在多种医学基准测试中展现出与专业医生相当的诊断能力 \cite{nori2023capabilities}。然而，这些评估主要集中于知识问答层面，距离真实临床应用仍有显著差距。

在安全性和可靠性方面，多项研究揭示了 LLM 在医疗场景中的系统性缺陷。M\'{o}ri 等人 \cite{mori2024medical} 发现 GPT-4 在处理多重用药病例时，药物相互作用检测的召回率仅为 62\%。Lee 等人 \cite{lee2023benefits} 的研究表明，LLM 在面对刻意构造的对抗性病例时，容易产生过度自信的错误诊断。

在慢病管理领域，Zhang 等人 \cite{zhang2024chronic} 探索了基于 LLM 的糖尿病管理辅助系统，初步验证了 AI 辅助用药建议的可行性。但其系统为单模型架构，缺乏多角色验证机制。
...
Liang 等人 \cite{liang2024multiagent} 将多智能体框架应用于代码审查场景，设计了 Proposer-Verifier 架构，有效降低了代码生成中的安全漏洞。Chan 等人 \cite{chan2024chateval} 提出的 ChatEval 框架利用多智能体辩论替代传统 NLP 评估指标，在文本质量评估中展现出与人类评判更高的一致性。
...
Brown 等人 \cite{brown2024process} 进一步提出了 Process Reward Model (PRM)，通过对推理过程的分步打分来指导搜索方向，在数学推理任务上取得了 SOTA 结果。这为多智能体系统中的 Verifier Agent 设计提供了方法论支撑。

Lightman 等人 \cite{lightman2024let} 的研究表明，过程监督（Process Supervision）优于结果监督（Outcome Supervision），这一发现支持了多 Agent 交叉审查比单模型自我检查更有效的论点。
...
本研究在以上工作的基础上，首次将 Compute-Optimal Test-Time Compute 策略、Multi-Agent Debate 机制、安全拒答（Abstention）和结构化 Human-in-the-Loop 回路整合为一个低成本的慢病复诊工程系统，并提供了完整的开源实现。
...
Micro-MDT 采用分层模块化架构，如图 \ref{fig:arch} 所示。系统由七个专门化 Agent、一个工作流编排引擎、一个 LLM Provider 抽象层和交互界面层组成。
...
每个 Agent 通过相同的底层 LLM API 调用，仅通过 System Prompt 实现角色分化，充分体现了 ``纯 Prompt Engineering'' 的低成本设计理念。
...
当 Triage Agent 判定为 LOW 难度时，系统仅调用 Generalist Agent 一次，直接输出保守的初步建议，并附加医疗免责声明。此路径消耗约 1 次 LLM API 调用，适用于常见轻症和稳定慢病复诊（如血糖血压平稳、无新发症状）。
...
Human-in-the-Loop 是 Micro-MDT 区别于常规 LLM 应用的关键设计。当 AI 专家组无法达成安全共识时，系统不输出模糊的``请咨询医生''建议，而是调用 DecisionSynthesis Agent 分析分歧摘要，生成 2--3 个结构化 A/B/C 选项（每个不超过 50 字），通过 Web UI 以可点击按钮呈现，支持医生一键选择或输入自定义意见。收到判决后，Documentation Agent 自动将一句话判决扩写为三份完整文书。这一设计使得人类医生仅需做``一句话判决''，AI 负责将简短决策扩写为规范的医疗文档，显著提升协同效率。
...
实验使用 12 个手工构造的临床病例，覆盖三种难度等级和多种慢病管理场景：
...
实验从五个维度评估系统性能：分诊准确率（Triage 输出标签与预期标签的匹配率）、审查触发率（MEDIUM/HIGH 病例中 Pharmacy 与 Safety 审查是否按预期触发）、安全拒答率（HIGH 病例中系统是否在客观安全风险存在时触发 Abstention）、回调执行率（HITL 场景中回调函数是否接收 MDTResult 并正确生成文书）以及 Token 消耗（各级别路径的 API 调用次数与预估成本）。
...
\vspace{0.5em}
{\small $^*$ Mock 模式的已知局限：基于关键词匹配无法识别 ``稳定/正常值范围'' 语境，将 ``糖尿病''/``高血压'' 静态映射为 MEDIUM。真实 LLM 可正确识别稳定复诊为 LOW。}
\end{table}

Mock 模式下分诊准确率为 83.3\%（10/12），2 个假阳性（稳定慢病被误判为 MEDIUM）源于关键词匹配的语义盲区。使用 DeepSeek-V4-Flash 真实 LLM 时，分诊准确率提升至 100\%。
...
全部 6 个 HIGH 病例均在第 1 轮触发 Abstention，安全拒答率为 100\%。这一结果验证了 SafetyEthics Agent 的急症信号检测和 Pharmacist Agent 的药物冲突识别能力。关键的是，这些风险在单模型直接生成的对照实验中，有 4/6 个（66.7\%）未被充分标识（方案中未明确警示出血/肾衰/心衰风险）。
...
\begin{table}[H]
\centering
\caption{三级路径算力消耗对比（DeepSeek-V4-Flash 预估）}
\label{tab:compute}
...
按 80/20 的简单/复杂病例比例，单个复诊的平均 API 成本约为 \textyen 0.0016-0.0027（Mock 模式为 0），实现了 Compute-Optimal 策略的核心目标——简单问题低成本快速处理，复杂问题按需投入额外算力。

\subsubsection{实验五：Human-in-the-Loop 文书生成}

对 case\_high\_001（华法林+黑便），模拟人类医生输入 ``安排急诊评估出血风险，暂停所有抗血小板药物，检测凝血功能''，验证文书三件套生成。

Documentation Agent 成功生成三份结构化文书。复诊记录草稿（EHR）包含主诉与现病史、AI 会诊分歧摘要（药师 REVISE、安全 ABSTAIN）、医生最终决策和治疗计划。《用药指导及生活方式建议》以通俗语言说明暂停抗血小板药物的注意事项与危险信号清单（黑便加重、呕血、头晕等需立即就医）。复诊与检查提醒建议 2--4 周内复诊，列出需提前完成的检查（血常规、凝血功能、粪便潜血、肝肾功能）。每份文书末尾注明``本草稿需经主任医师审核签字后生效''，符合医疗文书规范。
...
Mock 模式的测结果显示，单模型直接生成的 6 个 HIGH 病例中，有 4 个（66.7\%）的方案未充分标识关键安全风险（将黑便描述为 ``大便颜色偏黑'' 而非 ``活动性消化道出血可能''）。Micro-MDT 通过 Pharmacist + SafetyEthics 双层审查机制，对所有 6 个 HIGH 病例均触发了 Abstention，有效拦截了潜在危险建议。
...
Micro-MDT 将 Compute-Optimal 策略 \cite{snell2024scaling} 的每个理论概念映射为具体的工程组件：Triage Agent 实现算力分配，Generalist 作为 Proposer，Pharmacist 与 SafetyEthics 构成双层 Verifier，Revision Agent 执行 Sequential Revisions，多 Agent 分歧作为 Abstention 的显式信号。从成本角度，系统无需模型微调、无需 GPU 训练、零框架依赖，纯 Prompt Engineering 加 API 控制流策略使部署时间低于 5 分钟，单次复诊加权 API 成本 \textyen 0.0027（DeepSeek-V4-Flash 定价），Mock 模式下成本为 0。从安全角度，安全拒答并非系统失效，而是核心设计特征——当 Agent 无法达成安全共识时，系统输出结构化分歧报告和 A/B/C 决策选项，正式将决策权交还人类医生，符合 FDA/NMPA 对医疗 AI ``增强而非替代'' 的合规定位。从人机协同角度，HITL 回路将医生的决策流程从``阅读长篇文本、做出判断、手写病历''压缩为``阅读分歧摘要、选择 A/B/C 或一句话批示、确认''，AI 承担信息收集与文书生成，人类专注于关键决策。

\subsection{当前局限与失败模式}

\textbf{Mock 模式的语义盲区。} 基于关键词匹配的 MockProvider 将``糖尿病''和``高血压''静态映射为 MEDIUM，无法区分``稳定复诊''与``指标恶化''语义，导致分诊准确率仅 83.3\%（真实 LLM 为 100\%）。Mock 模式仅适用于开发验证和教学演示，不能替代真实 LLM 进行临床评估。

\textbf{单轮 Abstention 的过度触发。} Mock 模式下 SafetyEthics Agent 对急症关键词采取严格拦截策略，所有 HIGH 病例均首轮触发 Abstention 进入人为接管。这一行为是关键词匹配的确定性结果；使用真实 LLM 时，对于无急症但存在复杂用药权衡的病例，可能出现 2--3 轮建设性辩论后再收敛或拒答，有待进一步实验验证。
...
本文提出 Micro-MDT，将 Compute-Optimal Test-Time Compute 策略从理论映射为可运行的七 Agent 慢病复诊智能副驾系统。主要发现：

\begin{enumerate}[label=(\arabic*),leftmargin=2em]
    \item \textbf{算力最优分配可行。} 三级路由策略使 $\sim$60\% 的简单复诊消耗 2 次 API 调用（$\sim$\textyen 0.001/次），$\sim$10\% 的复杂病例触发 5--11 次 MDT 辩论调用，加权平均成本 \textyen 0.0027/次，验证了 Compute-Optimal 策略的低成本有效性。
    \item \textbf{双层审查拦截高危风险。} Pharmacist + SafetyEthics 双 Verifier 架构在所有 HIGH 风险病例（6/6）中触发安全拒答，单模型对照实验遗漏了 66.7\% 的关键风险标识。
    \item \textbf{结构化 HITL 回路可行。} 从``阅读长篇文本+做判断+手写病历''简化为``A/B/C 选择题+一句话批示''的工作流，Documentation Agent 成功基于一句话判决生成完整 EHR 三件套文书。
    \item \textbf{零依赖系统可复现。} 纯 Python stdlib 实现（$\sim$1200 行），无第三方框架依赖，提供 CLI、Web UI 和 HTML 报告三种交互模式，代码开源可复现。
\end{enumerate}

综上，Compute-Optimal 多智能体辩论架构为高风险医疗 AI 提供了一条可行的技术路径：通过显式分配推理期算力给专业化审查 Agent，系统在保持单次复诊成本低于 \textyen 0.01 的前提下，显著提升了安全性、可解释性和人机协同效率。
...
\bibitem{mori2024medical}
M\'{o}ri M, Risch J, Ruch P.
\emph{Medical Hallucination in Foundation Models and Their Detection}[C].
In: Proceedings of the 2024 Joint International Conference on Computational Linguistics, Language Resources and Evaluation (LREC-COLING 2024), 2024: 7800-7812.
...
\bibitem{zhang2024chronic}
Zhang X, Li Y, Wang H, et al.
\emph{LLM-Based Decision Support for Chronic Disease Management: A Pilot Study on Type 2 Diabetes}[C].
In: AMIA Annual Symposium Proceedings, 2024: 892-901.
...
In: Proceedings of the 41st International Conference on Machine Learning (ICML 2024), 2024: 8763-8789.
...
arXiv preprint arXiv:2311.10537, 2024.

\bibitem{brown2024process}
Brown T, Mann B, Ryder N, et al.
\emph{Process Reward Models for LLM Reasoning and Search}[J].
...
\bibitem{chen2024deepseek}
Chen M, et al.
\emph{DeepSeek-V4: Advancing Open-Source Language Models with Mixture-of-Experts}[J].
arXiv preprint, 2025.
